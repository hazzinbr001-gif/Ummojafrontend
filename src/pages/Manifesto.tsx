...
export default function Manifesto() {
  ...
  const onSubmit = (values: ManifestoForm) => {
    createItem.mutate({ data: values }, {
      onSuccess: () => {
        toast({ title: "Policy added to manifesto" });
        qc.invalidateQueries({ queryKey: getListManifestoItemsQueryKey() });
        form.reset();
        setOpen(false);
      },
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="gap-2"><Plus className="w-4 h-4" />Add Policy</Button>
          </DialogTrigger>
          <DialogContent>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
              ...
              <Button type="submit" className="w-full" disabled={createItem.isPending}>
                {createItem.isPending ? "Adding..." : "Add to Manifesto"}
              </Button>
            </form>
          </DialogContent>
        </Dialog>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        {CATEGORIES.map(c => {
          ...
          return (
            <Button key={c} variant={selectedCat === c ? "default" : "outline"} size="sm" onClick={() => setSelectedCat(selectedCat === c ? "" : c)}>
              {conf.label}
            </Button>
          );
        })}
      ...
      {isLoading ? (
        <div className="space-y-4">{[...Array(4)].map((_, i) => <div key={i} className="h-28 bg-muted rounded-xl animate-pulse" />)}</div>
      ) : (
