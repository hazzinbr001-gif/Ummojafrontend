...
const STATUS_LABELS: Record<string, string> = {
  pending: "Pending", registered: "Registered", attended_rally: "Attended Rally",
};
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm">
          ...
          {loadingLb ? (
            <div className="space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="h-12 bg-muted rounded-lg animate-pulse" />)}</div>
          ) : (
            <div className="space-y-2">
              ...
              {(!leaderboard || leaderboard.length === 0) && (
                <p className="text-sm text-muted-foreground text-center py-8">No referrals recorded yet</p>
              )}
            </div>
          )}
        </div>
                  ...
                  {referrals?.data.map((r) => (
                    <tr key={r.id} className="hover:bg-muted/30 transition-colors">
                      <td className="py-2.5 pr-3">
                        <div className="font-medium text-foreground text-xs">{r.referrerName}</div>
                        <div className="text-muted-foreground text-xs">{r.referrerPhone}</div>
                      </td>
                      <td className="py-2.5 pr-3">
                        <div className="text-xs text-foreground">{r.referredName || "—"}</div>
                      </td>
                      <td className="py-2.5 pr-3">
                        <span className={cn("px-1.5 py-0.5 rounded text-xs border font-medium", STATUS_COLORS[r.status] || "bg-gray-100 text-gray-600")}>
