...
import { useListVoters, useCreateVoter, useUpdateVoter, getListVotersQueryKey, ListVotersSupportLevel } from "@workspace/api-client-react";
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">IEBC register — Nyaribari Chache</p>
        </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2 space-y-1">
                  ...
                  <Input {...form.register("fullName")} placeholder="Full legal name" />
                </div>
                <div className="space-y-1">
                  <Label>ID Number</Label>
                  <Input {...form.register("idNumber")} placeholder="National ID" />
                </div>
                <div className="space-y-1">
                  <Label>Phone</Label>
                  <Input {...form.register("phone")} placeholder="07XX..." />
                </div>
                <div className="space-y-1">
                  <Label>Ward</Label>
                ...
                <div className="space-y-1">
                  <Label>Village</Label>
                  <Input {...form.register("village")} placeholder="Village name" />
                </div>
                <div className="col-span-2 space-y-1">
                  <Label>Polling Station</Label>
                  <Input {...form.register("pollingStation")} placeholder="Polling station name" />
                </div>
                <div className="col-span-2 space-y-1">
              ...
              <Button type="submit" className="w-full" disabled={createVoter.isPending}>
                {createVoter.isPending ? "Registering..." : "Register Voter"}
              </Button>
            </form>
          </DialogContent>
        </Dialog>
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? [...Array(10)].map((_, i) => (
                ...
                <tr key={voter.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={cn("text-xs font-medium", voter.hasVoted ? "text-green-700" : "text-muted-foreground")}>
                      {voter.hasVoted ? "Voted" : "Not yet"}
                    </span>
                  </td>
                </tr>
              ))}
        ...
        {data && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-border">
            <span className="text-sm text-muted-foreground">
              {data.total} voter{data.total !== 1 ? "s" : ""} total
            </span>
