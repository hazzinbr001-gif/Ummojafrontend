...
const STATUS_LABELS: Record<string, string> = {
  received: "Received", under_review: "Under Review", approved: "Approved", declined: "Declined", fulfilled: "Fulfilled",
};
...
export default function Requests() {
  ...
  const onSubmit = (values: RequestForm) => {
    createRequest.mutate({ data: values }, {
      onSuccess: () => {
        toast({ title: "Request submitted", description: "A reference number has been assigned." });
              ...
              {isLoading ? [...Array(8)].map((_, i) => (
                ...
                <tr key={req.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={cn("px-2 py-0.5 rounded-full text-xs border font-medium", STATUS_COLORS[req.status] || "bg-gray-100 text-gray-600")}>
                      {STATUS_LABELS[req.status]}
                    </span>
                  </td>
                  <td className="px-4 py-3 hidden sm:table-cell text-muted-foreground text-xs">
                    {new Date(req.createdAt).toLocaleDateString("en-KE", { month: "short", day: "numeric" })}
                  </td>
                  <td className="px-4 py-3">
