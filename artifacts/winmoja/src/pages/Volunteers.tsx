...
import { useListVolunteers, useCreateVolunteer, useUpdateVolunteer, getListVolunteersQueryKey } from "@workspace/api-client-react";
import type { Volunteer } from "@workspace/api-client-react";
...
const ROLE_LABELS: Record<string, string> = { ward_coordinator: "Ward Coordinator", polling_agent: "Polling Agent", mobilizer: "Mobilizer", driver: "Driver", other: "Other" };
...
export default function Volunteers() {
  ...
  const onSubmit = (values: VolunteerForm) => {
    createVolunteer.mutate({ data: values }, {
      onSuccess: () => { toast({ title: "Volunteer registered" }); qc.invalidateQueries({ queryKey: getListVolunteersQueryKey() }); form.reset(); setOpen(false); },
  ...
  const toggleActive = (id: number, isActive: boolean) => {
    updateVolunteer.mutate({ id, data: { isActive: !isActive } }, {
      onSuccess: () => { toast({ title: isActive ? "Volunteer deactivated" : "Volunteer activated" }); qc.invalidateQueries({ queryKey: getListVolunteersQueryKey() }); },
    });
  };
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button className="gap-2"><UserPlus className="w-4 h-4" />Register Volunteer</Button></DialogTrigger>
          <DialogContent>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
              ...
              <Button type="submit" className="w-full" disabled={createVolunteer.isPending}>{createVolunteer.isPending ? "Registering..." : "Register Volunteer"}</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
      ...
      {isLoading ? <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">{[...Array(6)].map((_, i) => <div key={i} className="h-24 bg-muted rounded-xl animate-pulse" />)}</div> : (
            ...
            return (
              <div key={w}>
                <h2 className="font-semibold text-foreground mb-3 flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-primary" />{w} Ward
                  ...
                  {wardVolunteers.map(v => (
                    <div key={v.id} className={cn("bg-card border border-border rounded-xl p-4 shadow-sm", !v.isActive && "opacity-60")}>
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div><div className="font-semibold text-sm text-foreground">{v.fullName}</div><div className="text-xs text-muted-foreground">{v.phone}</div></div>
                      ...
                      <span className={cn("px-2 py-0.5 rounded-full text-xs border font-medium", ROLE_COLORS[v.role] || "bg-gray-100 text-gray-600")}>{ROLE_LABELS[v.role]}</span>
                      {v.pollingStation && <div className="text-xs text-muted-foreground mt-2">{v.pollingStation}</div>}
                    </div>
                  ))}
                </div>
              </div>
