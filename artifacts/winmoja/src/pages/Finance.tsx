  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2"><h1 className="text-2xl font-bold text-foreground">Campaign Finance</h1><Lock className="w-4 h-4 text-muted-foreground" /></div>
          <p className="text-muted-foreground text-sm mt-0.5">Private — authorized team only</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button className="gap-2"><Plus className="w-4 h-4" />Log Transaction</Button></DialogTrigger>
          <DialogContent>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
              <div className="space-y-1"><Label>Type</Label>
                <Select defaultValue="income" onValueChange={v => { form.setValue("type", v as TxForm["type"]); form.setValue("category", ""); }}>
                  ...
                  <SelectContent><SelectItem value="income">Income</SelectItem><SelectItem value="expenditure">Expenditure</SelectItem></SelectContent>
                </Select>
              </div>
              <div className="space-y-1"><Label>Category</Label>
                <Select onValueChange={v => form.setValue("category", v)}>
                  <SelectTrigger><SelectValue placeholder="Select category" /></SelectTrigger>
                  <SelectContent>{(watchedType === "income" ? INCOME_CATEGORIES : EXPENDITURE_CATEGORIES).map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              ...
              <Button type="submit" className="w-full" disabled={createTransaction.isPending}>{createTransaction.isPending ? "Recording..." : "Record Transaction"}</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-4">
            <div className="w-10 h-10 rounded-lg bg-green-100 flex items-center justify-center"><TrendingUp className="w-5 h-5 text-green-700" /></div>
            <div><div className="text-xl font-bold text-green-800">KSh {summary.totalIncome.toLocaleString()}</div><div className="text-sm text-green-700">Total Income</div></div>
          </div>
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-4">
            <div className="w-10 h-10 rounded-lg bg-red-100 flex items-center justify-center"><TrendingDown className="w-5 h-5 text-red-700" /></div>
            <div><div className="text-xl font-bold text-red-800">KSh {summary.totalExpenditure.toLocaleString()}</div><div className="text-sm text-red-700">Total Expenditure</div></div>
          </div>
          <div className={cn("border rounded-xl p-4 flex items-center gap-4", summary.balance >= 0 ? "bg-primary/10 border-primary/30" : "bg-red-50 border-red-200")}>
            <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center", summary.balance >= 0 ? "bg-primary/20" : "bg-red-100")}>
              <DollarSign className={cn("w-5 h-5", summary.balance >= 0 ? "text-primary" : "text-red-700")} />
            </div>
            <div>
              <div className={cn("text-xl font-bold", summary.balance >= 0 ? "text-primary" : "text-red-800")}>KSh {Math.abs(summary.balance).toLocaleString()}</div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? [...Array(8)].map((_, i) => <tr key={i}><td colSpan={5} className="px-4 py-3"><div className="h-4 bg-muted rounded animate-pulse" /></td></tr>) :
                data?.data.map((tx) => (
                  <tr key={tx.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className={cn("flex items-center gap-1.5", tx.type === "income" ? "text-green-700" : "text-red-600")}>
                    ...
                    <td className={cn("px-4 py-3 text-right font-bold", tx.type === "income" ? "text-green-700" : "text-red-600")}>
                      {tx.type === "income" ? "+" : "-"}KSh {Number(tx.amount).toLocaleString()}
                    </td>
                    <td className="px-4 py-3 text-muted-foreground text-xs hidden sm:table-cell">{new Date(tx.createdAt).toLocaleDateString("en-KE", { month: "short", day: "numeric" })}</td>
                  </tr>
                ))}
            </tbody>
          </table>
